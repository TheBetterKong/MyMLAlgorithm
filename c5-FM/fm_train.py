...
# 定义损失函数 loss(y', y) = ∑-ln[sigmoid(y'* y)]
...
def stocGradient(dataMatrix, classLabels, k, max_iter, alpha):
    ...
    for it in range(max_iter):
        # print('第 %d 次迭代' % it)
        for x in range(m):
            v_1 = dataMatrix[x] * v    # dataMatrix[x] 的 shape 为 (1,n),v 的 shape 为 (n,k)--->v_1 的 shape 为 (1, k)
...
def main():
    # 第一步：导入数据
    feature, label = load_data('./data/train_data.txt')
    # print(feature, label)
    # 第二步：利用梯度下降训练模型
